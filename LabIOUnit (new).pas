...
begin
    case FNIDAQAPI of
        NIDAQMX : NIDAQMX_GetADCSamples( Device, ADCBuf, NumSamplesRead ) ;
        end ;
    end ;
...
begin
     ...
     // Allocate buffer
     //GetMem( DBuf, nChannels*nPoints*8*2 ) ;
    ...
    // Stop any running D/A task
     ...
     //CheckError( DAQmxCfgOutputBuffer ( DACTask[Device], nPoints )) ;

     // Convert to volts and ensure that data is within valid limits
     VScale := DACMaxVolts[Device] / (DACMaxValue[Device]+1.0) ;
 //    for i := 0 to nChannels*nPoints-1 do begin
 //        DBuf^[i] := Min(Max(DACBuf[i]*VScale,-10.0),10.0) ;
 //        end ;
     ...
     CheckError( DAQmxWriteBinaryI16{AnalogF64} ( DACTask[Device],
                           nPoints,
                           False,
                           DefaultTimeOut,
                           DAQmx_Val_GroupByScanNumber,
  ...
  //   outputdebugString(PANSIChar(format('DAQmxWriteAnalogF64 Dev=%d %d %d',[Device,nPoints,NumSamplesWritten]))) ;
